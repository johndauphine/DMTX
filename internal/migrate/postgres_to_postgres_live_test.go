...
type postgresSameDatabaseAliasObserver struct {
	tableSets   int
	beforeTable int
	afterTable  int
}

func (observer *postgresSameDatabaseAliasObserver) BeforeTables(
	context.Context,
	[]string,
) error {
	observer.tableSets++
...
func (observer *postgresSameDatabaseAliasObserver) BeforeTable(
	context.Context,
	string,
) error {
	observer.beforeTable++
	return nil
}

func (observer *postgresSameDatabaseAliasObserver) AfterTable(
	context.Context,
	string,
	int,
) error {
...
func TestPostgresToPostgresRejectsLiveSameDatabaseAlias(t *testing.T) {
	dsn := os.Getenv("DMTX_TEST_POSTGRES_DSN")
	if dsn == "" {
		t.Skip(
			"set DMTX_TEST_POSTGRES_DSN to run the PostgreSQL live same-database sentinel",
		)
	}
	parsed, err := pgx.ParseConfig(dsn)
	if err != nil {
		t.Fatalf("parse PostgreSQL same-database sentinel DSN: %T", err)
	}
	...
	default:
		t.Skip("PostgreSQL live alias sentinel requires a loopback DSN")
	}
	...
	if err != nil {
		t.Fatalf("open PostgreSQL same-database sentinel: %T", err)
	}
	t.Cleanup(func() {
		if err := database.Close(); err != nil {
			t.Errorf("close PostgreSQL same-database sentinel: %v", err)
		}
	})
	if err := database.PingContext(ctx); err != nil {
		t.Fatalf("verify PostgreSQL same-database sentinel: %T", err)
	}
	...
	tableName := "identity_sentinel"
	if _, err := database.ExecContext(
		ctx,
		"CREATE SCHEMA "+postgresIdentifier(namespace),
	); err != nil {
		t.Fatalf("create PostgreSQL same-database sentinel schema: %v", err)
	}
	t.Cleanup(func() {
		...
		if _, err := database.ExecContext(
			...
			t.Errorf(
				"drop PostgreSQL same-database sentinel schema: %v",
				err,
			)
		}
	})
	qualifiedTable := postgresQualified(namespace, tableName)
	if _, err := database.ExecContext(
		ctx,
		"CREATE TABLE "+qualifiedTable+` (
			id bigint GENERATED BY DEFAULT AS IDENTITY
				(START WITH 41) PRIMARY KEY,
			payload text NOT NULL
		)`,
	); err != nil {
		t.Fatalf("create PostgreSQL same-database sentinel table: %v", err)
	}
	if _, err := database.ExecContext(
		ctx,
		"INSERT INTO "+qualifiedTable+
			" (payload) VALUES ('must survive')",
	); err != nil {
		t.Fatalf("seed PostgreSQL same-database sentinel: %v", err)
	}
	var (
		sequenceName string
		lastValue    int64
	...
	if err := database.QueryRowContext(
		...
		t.Fatalf("read PostgreSQL sentinel identity sequence: %v", err)
	}
	if err := database.QueryRowContext(
		ctx,
		"SELECT last_value, is_called FROM "+sequenceName,
	).Scan(&lastValue, &isCalled); err != nil {
		t.Fatalf("read PostgreSQL sentinel identity frontier: %v", err)
	}
	...
	targetEndpoint.Host = targetHost
	observer := &postgresSameDatabaseAliasObserver{}
	...
	if err == nil ||
		...
		t.Fatalf(
			"same live PostgreSQL database result = %+v, error = %v",
			result,
			err,
		)
	}
	if result != (Result{}) {
		t.Fatalf("same-database result = %+v, want zero", result)
	}
	if observer.tableSets != 0 ||
		observer.beforeTable != 0 ||
		observer.afterTable != 0 {
		t.Fatalf(
			"same-database observer calls = sets:%d before:%d after:%d, want zero",
			observer.tableSets,
			observer.beforeTable,
			observer.afterTable,
		)
	...
	var (
		id              int64
		payload         string
		finalLastValue  int64
		finalSequenceOK bool
	)
	if err := database.QueryRowContext(
		ctx,
		"SELECT id, payload FROM "+qualifiedTable,
	).Scan(&id, &payload); err != nil {
		t.Fatalf("read preserved PostgreSQL sentinel row: %v", err)
	}
	if id != 41 || payload != "must survive" {
		t.Fatalf(
			"preserved PostgreSQL sentinel row = (%d, %q), want (41, %q)",
			id,
			payload,
			"must survive",
		)
	...
	if err := database.QueryRowContext(
		ctx,
		"SELECT last_value, is_called FROM "+sequenceName,
	).Scan(&finalLastValue, &finalSequenceOK); err != nil {
		t.Fatalf("read preserved PostgreSQL identity frontier: %v", err)
	}
	if finalLastValue != lastValue || finalSequenceOK != isCalled {
		t.Fatalf(
			"PostgreSQL identity frontier = (%d, %t), want (%d, %t)",
			finalLastValue,
			finalSequenceOK,
			lastValue,
			isCalled,
