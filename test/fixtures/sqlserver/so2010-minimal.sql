-- Copied verbatim from dmt (scripts/fixtures/so2010-minimal.sql), which is
-- the same author's project and where this fixture has been the per-PR
-- integration corpus. Kept byte-identical on purpose: the point is that both
-- tools are proved against the same schema, and a fixture that drifted would
-- quietly stop being a comparison.
--
-- dmtx used to test SQL Server against a schema of its own devising, whose text
-- columns all carried COLLATE Latin1_General_100_BIN2_UTF8 and whose timestamps
-- were all datetime2. Both were the only spellings the discovery code accepted.
-- Fourteen minutes of armed CI passed over a route that could not read an
-- ordinary StackOverflow table, because the fixture had been shaped to fit the
-- implementation rather than the world.
--
