-- Copied verbatim from dmt (scripts/fixtures/so2010-minimal-sqlite.sql), where it
-- is the corpus its nightly cross-engine matrix seeds this engine from. Kept
-- byte-identical on purpose: the point is that both tools are proved against
-- the same schema, and a fixture that drifted would quietly stop being a
-- comparison.
--
-- Seeded independently rather than produced by migrating another engine's copy.
-- A chained corpus is written BY dmtx, so a wrong mapping would be baked into
-- the next hop's input and validated against itself - which is the shape of the
-- defect that hid an unreadable nvarchar column behind fourteen minutes of
-- green CI. A fixture dmtx did not author is the only kind that can find dmtx's
-- blind spots.
--
